...
if ( !function_exists( 'skeleton_customizer_styles' ) ) {
    ...
    function skeleton_customizer_styles() {
        ...
        $menu_font = str_replace('+', ' ', skeleton_options('menu_font', 'Sans-Serif'));
        $protocol = is_ssl() ? 'https' : 'http';

        $font_query_args = [];

        if ($body_font != 'Sans-Serif' && $body_font != 'Serif') {
            $font_query_args = ['family' => skeleton_options('body_font').':400,700'];
        }

        if ($heading_font != 'Sans-Serif' && $heading_font != 'Serif') {
            $font =  str_replace(' ', '+', $heading_font);
            if (empty($font_query_args['family'])) {
                $font_query_args = ['family' => $font.':400,700'];
            } elseif (false === strpos($font_query_args['family'], $font)) {
                 $font_query_args= ['family' => $font_query_args['family'].'|'.$font];
            }
        }

        if ($menu_font != 'Sans-Serif' && $menu_font != 'Serif') {
            $font =  str_replace(' ', '+', $menu_font);
            if (empty($font_query_args['family'])) {
                $font_query_args = ['family' => $font.':400,700'];
            } elseif (false === strpos($font_query_args['family'], $font)) {
                 $font_query_args =['family' => $font_query_args['family'].'|'.$font];
        ...
        if (!empty($font_query_args)) {
            wp_enqueue_style('skeleton-heading-fonts',add_query_arg($font_query_args, "$protocol://fonts.googleapis.com/css" ), [], null);
        }
...
function skeleton_customize_register( $wp_customize ) {
    ...
    $wp_customize->add_setting( 'skeleton_options[menu_font]', array(
        'capability' => 'edit_theme_options',
        'default'    => 'Sans-Serif',
        'sanitize_callback' => 'sanitize_text_field',
        'type'       => 'option'
    ...
    $wp_customize->add_control( 'menu_font', array(
        'settings' => 'skeleton_options[menu_font]',
        'label'    => __( 'Menu Font:', 'smpl' ),
        'section'  => 'skeleton_fonts',
        'type'     => 'select',
        'choices'  => $available_fonts
    ));
